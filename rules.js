// rules.js - Enterprise Knowledge Base v3.9 (Bilingual Precision Guard & Extended Entity Boundaries)

(function(global) {
    'use strict';

    // 台灣身分證格式校驗 (結構格式比對，兼顧測試假號碼與真實字號)
    ...
    const rules = {
        structured: [
            ...
            // 4. Passports & Government IDs (國際護照優先於 HKID，防止 8 碼字串截斷)
            ...
            // 6. Title-Prefixed Contextual English Names (支援 O'Brien, McDonald, Jean-Luc, J. R.)
            { id: 'ENGLISH_NAME', tokenPrefix: 'EMPLOYEE_NAME', regex: /\b(?:Director|Manager|Patient|Doctor|Dr\.|Mr\.|Ms\.|Mrs\.|Officer|Contact|President|CEO|Vice President)\s+[A-Z][A-Za-z.'-]{1,20}(?:\s+[A-Z][A-Za-z.'-]{1,20}){1,2}\b/g },

            // 7. Universal Phone Numbers (隔離前後英數字，保護卡號與證件號)
        ...
        organizational: [
            ...
            { id: 'ADDRESS_EN', tokenPrefix: 'ADDRESS_LOCATION', regex: /\b\d{1,5}\s+[A-Z][a-zA-Z0-9\s.,]{2,30}(?:Street|St\.|Avenue|Ave\.|Road|Rd\.|Boulevard|Blvd\.|Drive|Dr\.|Way|Lane|Ln\.)(?:,\s*(?:Apt|Suite|Floor|Building|\d+[A-Z]?)\s*\d*)?(?:,\s*[A-Za-z\s]+)?\b/g },
